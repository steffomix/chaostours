...
class ModelUserGroup extends Model {
  ...
  static ModelUserGroup _fromMap(Map<String, Object?> map) {
  ...
  static Future<int> update(ModelUserGroup model) async {
    if (model.id <= 0) {
      throw ('update model "${model.title}" has no id');
    }
    var count = await DB.execute<int>(
      (Transaction txn) async {
        return await txn.update(TableUserGroup.table, model.toMap());
      },
    );
    return count;
  }
